...
public class QuestionFetcher
{
    ...
    public string? CreatePdf()
    {
        try
        {
            ...
            return AppSettings.OutputPdfPath;
        }
        catch (Exception ex)
        {
    ...
    {
        var onlyAcceptedAnswers = AppSettings.OnlyAcceptedAnswers.HasValue && AppSettings.OnlyAcceptedAnswers.Value ? "AND a.\"IsAccepted\" = true" : "";
        var limitCount = AppSettings.MaxRecordCount.HasValue ? "LIMIT " + AppSettings.MaxRecordCount.Value : "";
...
@" ORDER BY q.""Number"", a.""CreationTime"" " + limitCount;
    ...
    private static void AddFirstPage(Document document)
    {
        var s = "THIS DOCUMENT CONTAINS Q&A " + Environment.NewLine +
               ...
               (AppSettings.OnlyAcceptedAnswers.HasValue && AppSettings.OnlyAcceptedAnswers.Value ? "ONLY ACCEPTED ANSWERS ARE INCLUDED " : "") + Environment.NewLine +
